...
// Every gallery shot is a full 1600x900 page capture, so the part that proves the
// claim is only a fraction of the frame. Crop to that region (source pixels) so the
// UI stays legible at landing-page sizes instead of shrinking to unreadable mush.
const SHOT = { w: 1600, h: 900 } as const;

type Crop = { x: number; y: number; w: number; h: number };

// Both side-by-side shots share one crop aspect so the two windows stay the same height.
// x starts just left of the host's content column so nothing clips at the edge.
...
function Window({ src, alt, url, eager = false, className, crop }: { src: string; alt: string; url: string; eager?: boolean; className?: string; crop?: Crop }) {
  return (
    <figure className={cn("overflow-hidden rounded-[15px] border bg-card", className)}>
      ...
      {crop ? (
        // Scale the full shot up so `crop` exactly fills the box, then offset it into view.
