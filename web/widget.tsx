    ...
    <div className="flex h-dvh flex-col bg-background">
      <header className="flex h-12 flex-none items-center justify-between border-b px-4">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold tracking-tight">MissionCart Demo Host</span>
            ...
            <>
              ...
              {phase === "failed" && (
                <div className="mt-4 text-[15px] leading-7">
                  <p>I couldn’t reach the MissionCart backend: {error}</p>
              ...
              {phase === "ready" && view && (
                <>
                  ...
                  {showWidget && (
                    <div className="rise-in mt-6">
                      <MissionCart view={view} setView={setView} nonce={nonce} invoke={invoke} />
                    </div>
                  )}
                  {laterCalls.length > 0 && (
  ...
  return (
    <main className="mx-auto w-full max-w-[1040px] overflow-hidden rounded-2xl border bg-background shadow-sm">
      <header className="relative overflow-hidden bg-zinc-950 px-6 py-8 text-white sm:px-10 sm:py-10">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold tracking-tight">MissionCart</span>
      ...
      <section className="px-6 py-8 sm:px-10" aria-labelledby="results-heading">
        <Kicker number="01" label={`${view.carts.length} complete carts`} />
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 id="results-heading" className="text-xl font-semibold tracking-tight">Ready for pickup</h2>
            <p className="mt-1 text-sm text-muted-foreground">Every option passes the mission’s hard constraints.</p>
          </div>
      ...
      {activeCart && (
        <section className="border-t px-6 py-8 sm:px-10">
          <Kicker number="02" label="Compatibility proof" />
          <div className="mt-3 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
            <div>
              <h2 className="text-xl font-semibold tracking-tight">Why this kit works</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                MissionCart verifies the complete setup, not just individual search results.
              </p>
      ...
      {preview && !view.order && (
        <section className="border-t px-6 py-8 sm:px-10" aria-labelledby="checkout-heading">
          <Kicker number="03" label="Explicit confirmation" />
          <div className="mt-5 grid overflow-hidden rounded-xl border lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="flex flex-col justify-center border-t bg-muted/40 p-6 sm:p-8 lg:border-l lg:border-t-0">
              ...
              <h3 className="mt-3 text-base font-semibold tracking-tight">Bound, expiring, one-time</h3>
              <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                No card credentials enter MissionCart. The demo returns a simulated network authorization.
              </p>
...
function Loading({ message }: { message: string }) {
  return (
    <main className="mx-auto grid min-h-[280px] w-full max-w-[1040px] place-items-center content-center gap-2 rounded-2xl border bg-background p-10 text-center shadow-sm">
      ...
      <h1 className="mt-2 text-sm font-semibold tracking-tight">MissionCart</h1>
