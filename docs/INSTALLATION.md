...
Developers who intentionally provide an existing key can run the opt-in live
evaluation separately from CI:

```bash
npm run eval:agent
```

The command covers home-office, photography, and cooking missions. Web results
remain cited research-only leads, never checkout-capable offers.
...
- Its details page shows the Flightpath icon, Woven wordmark, and three verified
  ...
  pickup receipt.
- The new task can access nine Woven tools:
  `start_mission`, `build_carts`, `select_cart`,
  `swap_cart_item`, `start_demo_identity`, `create_checkout_preview`,
  `confirm_purchase`, `get_order_status`, and `verify_receipt`.
- The prompt opens Woven's Choice Center with five complete carts across
  Central, East, and North pickup areas.
- TrailHaus is the **Best match** and CampWorks is the **Best value**.
- Selecting a cart changes no inventory and makes no purchase.
- Comparison, priority reranking, pickup planning, and merchant-approved swaps
  remain inside the same complete-cart boundary.
- **Review checkout** first requires the simulated demo identity handoff, then
  creates an exact ten-minute preview.
- Only the widget's separate **Confirm S$231.00** action can produce the clearly
...
```json
{"ok":true,"service":"woven","version":"0.3.0","paymentMode":"simulated"}
```
...
Expected result: ChatGPT discovers the same nine tools and renders the same
...
| `start_mission` succeeds but the widget area is blank | Run `npm run check`, reinstall Woven 0.3.0 or newer, then start a new task | The self-contained Choice Center opens with five interactive cart choices |
| **I've verified · check status** appears unchanged | Update to Woven 0.3.0, refresh/reinstall the plugin, then start a new task | The current widget updates to verified or reopens the authorization handoff |
