...
| Public HTTPS deployment | Complete for the demo | <https://visa-woven.vercel.app> serves `/`, `/demo`, `/merchant`, `/healthz`, and `/mcp` through Vercel's Express preset; SQLite uses temporary `/tmp` state and may reset on cold start or redeploy |
